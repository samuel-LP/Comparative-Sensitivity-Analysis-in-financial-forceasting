...
class PortfolioPredicitionsLSTM_return():
...
class PortfolioPredicitionsLSTMVolatility2():
    def __init__(self, stock_dict, n_steps, epochs):
        self.stock_dict = stock_dict
        self.splitter = TimeSeriesSplit(n_splits=2)
        self.n_steps = n_steps
    ...
    def compute_portfolio_value(self, stock):
        ...
        temp["Volatility"] = self.returns_test[self.n_steps+6:]
        temp["Prediction"] = self.test_predictions[6:]
        temp.drop(columns="Close", inplace=True)
...
class PortfolioPredicitionsLSTM_diff2():
    def __init__(self, stock_dict, n_steps, epochs):
        self.stock_dict = stock_dict
        self.splitter = TimeSeriesSplit(n_splits=2)
        self.n_steps = n_steps
    ...
    def split_data(self, stock):
        returns = stock['Close'].diff().values.reshape(-1, 1)
        self.train, self.test = self.splitter.split(returns)
    ...
    def compute_portfolio_value(self, stock):
        ...
        temp["Prediction"] = self.test_predictions
        temp["Prediction"] = temp["Close"].iloc[0] + temp["Prediction"].cumsum()
